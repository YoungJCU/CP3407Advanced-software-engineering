// map2.js - enhanced map UI: building overview, clickable blocks, level and room details
(function(){
  // Wait for DOM to be ready
  document.addEventListener('DOMContentLoaded', () => {
    // Basic references (same IDs as index.html)
    ...
    // Load campus JSON (prefer mapped)
    ...
    function renderLevelsForBuilding(building, campusData){
      // populate building select and levels
      ...
      // select first level and render rooms
      if(building.levels && building.levels.length) renderRoomList(building.levels[0], campusData);
    }

    function renderRoomList(level, campusData){
      if(!roomList) return;
      roomList.innerHTML = '';
    ...
    function renderSvgForLevel(level){
      ...
      rooms.forEach((r, idx) => {
        ...
        rect.style.cursor='pointer'; rect.addEventListener('click', async () => {
          // try backend detail first if id
          if(r.id){
            try{ const resp = await window.getClassroomById && window.getClassroomById(r.id); if(resp) showBackendDetails(resp); else showLocalDetails(r, level); } catch(e){ showLocalDetails(r, level); }
