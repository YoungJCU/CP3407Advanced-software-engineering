...
- **Login:** `POST /api/auth/login` — checks email + password against SQLite `users`.
- **Register:** `POST /api/auth/register` — saves a new email + password (role `student`).
- On the login page, the app **tries login first**; if the server returns 401 (no match), it **tries register** so first-time visitors create an account; if register returns 409, the email already exists and the password was wrong.
- **Profile:** `GET /api/auth/user?email=...` — used by the home page header.
- **Cleanup:** On each server start, invalid rows (bad email / empty password) and **duplicate emails** (same address twice) are removed from `users` to limit redundant data.

Demo accounts are still seeded on startup:

- `student1@jcu.edu.sg` / `Password123`
- `teacher1@jcu.edu.sg` / `Password123`
- `admin@jcu.edu.sg` / `Password123`

Passwords are stored as plain text in SQLite for this coursework demo (not production-safe). The browser also caches the signed-in email and display name in `localStorage` for quick UI refresh.
