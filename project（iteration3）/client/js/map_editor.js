...
(function () {
  ...
  function dataForSave() {
    const blocks = (data.blocks || []).map((b) => {
      const o = {
        ...
        ...(b.h_pct != null && Number.isFinite(Number(b.h_pct)) ? { h_pct: Number(b.h_pct) } : {}),
  ...
  function onBlockInput(ev) {
    ...
    if (k === 'left_pct' || k === 'top_pct') {
      data.blocks[i][k] = Number(inp.value) || 0;
    } else if (k === 'w_pct' || k === 'h_pct' || k === 'rot_deg') {
      const raw = String(inp.value || '').trim();
      if (!raw) delete data.blocks[i][k];
      else {
        const v = Number(raw);
        if (Number.isFinite(v)) data.blocks[i][k] = v;
      }
  ...
  function onPointerMove(ev) {
    ...
    const { w, h } = syncViewBox();
    const minW = Math.max(12, w * 0.005);
    const minH = Math.max(10, h * 0.005);
    const nextW = Math.max(minW, resizeState.startW + dx);
    const nextH = Math.max(minH, resizeState.startH + dy);
    const nextWPct = Number(((nextW / w) * 100).toFixed(2));
    const nextHPct = Number(((nextH / h) * 100).toFixed(2));
    if (resizeState.type === 'block') {
      const b = data.blocks[resizeState.i];
      if (!b) return;
      b.w_pct = nextWPct;
      b.h_pct = nextHPct;
      renderBlockForm();
    } else if (resizeState.type === 'gate') {
      data.gate.w_pct = nextWPct;
      data.gate.h_pct = nextHPct;
      gateInputs();
    }
    drawPreview();
  }
  ...
  function endResize(ev) {
    try {
      if (resizeState && resizeState.captureTarget && ev && ev.pointerId && resizeState.captureTarget.releasePointerCapture) {
        resizeState.captureTarget.releasePointerCapture(ev.pointerId);
      }
    } catch (e) {}
