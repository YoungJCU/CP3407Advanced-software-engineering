...
db.serialize(() => {
  ...
  const seedUsers = [
    ['student1@jcu.edu.sg', 'Student One', 'student', 'Password123'],
    ['teacher1@jcu.edu.sg', 'Teacher One', 'lecturer', 'Password123'],
    ['admin@jcu.edu.sg', 'Campus Admin', 'admin', 'Password123'],
  ];
  ...
  seedUsers.forEach((u) => upsertUser.run(u[0], u[1], u[2], u[3]));
  upsertUser.finalize();
...
/** After DB is ready: drop junk rows and duplicate emails (same account stored twice). */
setImmediate(() => {
  try {
    const userModel = require('../models/userModel');
    userModel
      .deleteInvalidRows()
      .then((a) => userModel.dedupeByEmailLower().then((b) => ({ a, b })))
      .then(({ a, b }) => {
        if ((a && a.deleted) || (b && b.deleted)) {
          console.log(
            `Users table cleanup: removed ${a ? a.deleted : 0} invalid row(s), ${b ? b.deleted : 0} duplicate email row(s).`
          );
        }
      })
      .catch((e) => console.warn('Users table cleanup:', e.message));
  } catch (e) {
    console.warn('Users table cleanup load failed', e.message);
  }
});
