...
function validateBody(data) {
  ...
  if (data.gate.w_pct != null) {
    const gw = Number(data.gate.w_pct);
    if (!Number.isFinite(gw) || gw < 0.1 || gw > 100) return 'gate.w_pct must be a number between 0.1 and 100';
  }
  if (data.gate.h_pct != null) {
    const gh = Number(data.gate.h_pct);
    if (!Number.isFinite(gh) || gh < 0.1 || gh > 100) return 'gate.h_pct must be a number between 0.1 and 100';
  }
